...
import { getStudentExams } from "../../../../service/GlobalApi";
  ...
  useEffect(() => {
    ...
    const fetchExams = async () => {
      try {
        ...
        fetchSubmissionStatus(res.data, user);
      } catch (err) {
        console.error(err);
      }
    };
      ...
      for (let exam of exams) {
        try {
          const res = await fetch(
            `http://localhost:5000/api/marks/check?student_id=${user.id}&semester=${exam.semester}&exam_number=${exam.test_no}`,
          );
